...
from predictor import predict_deepfake, aggregate_deepfake_session

app = FastAPI(title="Faceo Analytics — Deepfake Service")
...
async def analyze_live_session(frames: list[UploadFile] = File(...)):
    ...
    for frame_file in frames:
        ...
        img = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
        if img is not None:
            frame_results.append(predict_deepfake(img))

    aggregated = aggregate_deepfake_session(frame_results)
